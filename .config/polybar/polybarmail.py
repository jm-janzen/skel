...
parser.add_argument('-dr', '--duration', type=int, default=120)
parser.add_argument('-hz', '--hide-if-zero', action='store_true', default=True)
...
def update_count(count_was):
    ...
    if config.getboolean('default', 'hide_if_zero'):
        if count > 0:
            subprocess.run(['polybar-msg', 'action', 'mail', 'module_show'],
                           stdout=subprocess.DEVNULL)
        else:
            subprocess.run(['polybar-msg', 'action', 'mail', 'module_hide'],
                           stdout=subprocess.DEVNULL)

    if not config['default']['nosound'] and count_was < count and count > 0:
...
while True:
    try:
        ...
        time.sleep(config.getint('default', 'duration'))
